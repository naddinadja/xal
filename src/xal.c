 ...
 * carried in this region, so no out-of-band communication is required.
 *
 * On failure nothing is left behind under the name; the caller still owns the xal.
 */
static int
...
xal_open_from_uri(const char *uri, struct xal **xal, struct xal_opts *opts)
{
	...
	err = attach_or_claim_shm(NULL, xal, opts, &attached);
	if (err) {
		return err;
	}
	if (attached) {
	...
	/* Without a device there is nothing to read the on-disk format from, so FIEMAP is the
	 * only backend reachable here; anything else is a caller mistake worth naming. */
