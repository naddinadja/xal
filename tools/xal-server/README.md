...
The server logs to syslog under the identity `xal-server`, exits on SIGTERM/SIGINT, and closes
each index on the way out -- which unlinks the shared memory. Readers must be done before then.
