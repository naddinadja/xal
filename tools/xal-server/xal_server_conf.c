 ...
 *
 * Truncating here would silently produce a different path or shared memory name than the one the
 * operator wrote, so a too-long value is an error rather than a warning.
 */
static int
...
parse_devices(toml_datum_t devices, struct xal_server_conf *conf)
{
	...
	for (unsigned int i = 0; i < conf->ndevs; i++) {
		...
		/* Without an IPC channel the shared memory name is the whole contract between this
		 * server and its readers, so it is spelled out here rather than derived from a
		 * position in the device list that a later edit could shift. */
		...
		/* POSIX wants a shared memory name to be a leading slash followed by no further
		 * ones. Left to shm_open() a malformed name surfaces as a bare errno from deep in
		 * the open path, with nothing tying it back to this device entry. */
		if ((dev->shm_name[0] != '/') || strchr(dev->shm_name + 1, '/')) {
			syslog(LOG_ERR,
			       "device %u: 'shm_name' (%s) must be a leading '/' followed by no "
			       "further '/'",
			       i, dev->shm_name);
			return -EINVAL;
		}
		...
		for (unsigned int j = 0; j < i; j++) {
			...
			/* The second open of a name attaches as a secondary instead of publishing,
			 * so a duplicate would leave one device unpublished with no obvious
			 * symptom. */
...
{
	/* Config-file verbosity, least to most; syslog's own numbering is not contiguous. The
	 * quietest setting is LOG_ERR rather than LOG_CRIT so that no configuration can suppress
	 * the errors explaining why the server refused to start. */
